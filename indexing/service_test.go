...
import (
	...
	"github.com/ethereum/go-ethereum/indexing/testhelpers/mocks"
)
...
func testErrorInChainEventLoop(t *testing.T) {
	...
	service := indexing.Service{
		...
		Subscriptions:     make(map[common.Hash]map[rpc.ID]indexing.Subscription),
		SubscriptionTypes: make(map[common.Hash]indexing.Params),
	}
	payloadChan := make(chan indexing.Payload, 2)
	...
	payloads := make([]indexing.Payload, 0, 2)
...
func testErrorInBlockLoop(t *testing.T) {
	...
	payloadChan := make(chan indexing.Payload)
	quitChan := make(chan bool)
	service.Subscribe(rpc.NewID(), payloadChan, quitChan, defaultParams)
	blockMapping := make(map[common.Hash]*types.Block)
	blockMapping[parentBlock1.Hash()] = parentBlock1
...
func testErrorInStateDiffAt(t *testing.T) {
	mockStateDiff := indexing.StateObject{
	...
	expectedStateDiffPayload := indexing.Payload{
