...
type MockStateDiffService struct {
	...
	QuitChan          chan bool
	Subscriptions     map[common.Hash]map[rpc.ID]indexing.Subscription
	SubscriptionTypes map[common.Hash]indexing.Params
}
...
func (sds *MockStateDiffService) APIs() []rpc.API {
	return []rpc.API{
		{
			Namespace: indexing.APIName,
			Version:   indexing.APIVersion,
			Service:   indexing.NewPublicStateDiffAPI(sds),
			Public:    true,
		},
	}
}
...
func (sds *MockStateDiffService) StateDiffAt(blockNumber uint64, params indexing.Params) (*indexing.Payload, error) {
...
func (sds *MockStateDiffService) processStateDiff(currentBlock *types.Block, parentRoot common.Hash, params indexing.Params) (*indexing.Payload, error) {
	stateDiff, err := sds.Builder.BuildStateDiffObject(indexing.Args{
...
func (sds *MockStateDiffService) newPayload(stateObject []byte, block *types.Block, params indexing.Params) (*indexing.Payload, error) {
...
func (sds *MockStateDiffService) StateTrieAt(blockNumber uint64, params indexing.Params) (*indexing.Payload, error) {
	currentBlock := sds.BlockChain.GetBlockByNumber(blockNumber)
	log.Info(fmt.Sprintf("sending state trie at %d", blockNumber))
	return sds.stateTrieAt(currentBlock, params)
}

func (sds *MockStateDiffService) stateTrieAt(block *types.Block, params indexing.Params) (*indexing.Payload, error) {
...
func (sds *MockStateDiffService) Subscribe(id rpc.ID, sub chan<- indexing.Payload, quitChan chan<- bool, params indexing.Params) {
	...
	if sds.Subscriptions[subscriptionType] == nil {
		sds.Subscriptions[subscriptionType] = make(map[rpc.ID]indexing.Subscription)
	}
	sds.Subscriptions[subscriptionType][id] = indexing.Subscription{
		PayloadChan: sub,
		QuitChan:    quitChan,
	}
...
func sendNonBlockingQuit(id rpc.ID, sub indexing.Subscription) {
	select {
	case sub.QuitChan <- true:
		log.Info(fmt.Sprintf("closing subscription %s", id))
	default:
