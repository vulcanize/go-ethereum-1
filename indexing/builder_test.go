...
import (
	...
	"github.com/ethereum/go-ethereum/indexing/testhelpers"
)

// TODO: add test that filters on address
var (
	contractLeafKey                                        []byte
	emptyDiffs                                                                                          = make([]indexing.StateNode, 0)
	emptyStorage                                                                                      = make([]indexing.StorageNode, 0)
...
func TestBuilder(t *testing.T) {
	...
	builder = indexing.NewBuilder(chain.StateCache())

	var tests = []struct {
		name              string
		startingArguments indexing.Args
		expected          *indexing.StateObject
	}{
		{
			"testEmptyDiff",
			indexing.Args{
		...
		},
		{
			...
			&indexing.StateObject{
				BlockNumber: block0.Number(),
				BlockHash:   block0.Hash(),
				Nodes: []indexing.StateNode{
					{
						Path:         []byte{},
						NodeType:     indexing.Leaf,
			...
			&indexing.StateObject{
				...
				Nodes: []indexing.StateNode{
					...
					},
					{
						...
						StorageNodes: []indexing.StorageNode{
							{
								Path:      []byte{'\x02'},
								NodeType:  indexing.Leaf,
