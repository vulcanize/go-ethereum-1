...
type Database struct {
	pgdb      *sqlx.DB
	lvldb     *leveldb.Database
	l         *lvldb.DB
	ancientTx *sqlx.Tx
}

// NewDatabase returns a ethdb.Database interface for PG-IPFS
func NewDatabase(db *sqlx.DB, file string, cache int, handles int, namespace string) (ethdb.Database, error) {
	ldatabase, err := leveldb.New(file, cache, handles, namespace)
	if err != nil {
		return nil, err
	}
	l := ldatabase.ExposeDB()
	ldb := l.(*lvldb.DB)
	return &Database{
		pgdb:  db,
		lvldb: ldatabase,
...
func (d *Database) Has(key []byte) (bool, error) {
	...
	var exists bool
	return exists, d.pgdb.Get(&exists, pgStr, key)
}
...
func (d *Database) Get(key []byte) ([]byte, error) {
	...
	var data []byte
	return data, d.pgdb.Get(&data, pgStr, key)
}
...
func (d *Database) Put(key []byte, value []byte) error {
	...
	if _, err = d.pgdb.Exec(pgStr, args...); err != nil {
		return err
	}
	return nil
}
...
func (d *Database) Stat(property string) (string, error) {
	...
	case Size:
		var byteSize string
		return byteSize, d.pgdb.Get(&byteSize, dbSizePgStr)
	case Idle:
		return string(d.pgdb.Stats().Idle), nil
	case InUse:
		return string(d.pgdb.Stats().InUse), nil
	case MaxIdleClosed:
		return string(d.pgdb.Stats().MaxIdleClosed), nil
	case MaxLifetimeClosed:
		return string(d.pgdb.Stats().MaxLifetimeClosed), nil
	case MaxOpenConnections:
		return string(d.pgdb.Stats().MaxOpenConnections), nil
	case OpenConnections:
		return string(d.pgdb.Stats().OpenConnections), nil
	case WaitCount:
		return string(d.pgdb.Stats().WaitCount), nil
	case WaitDuration:
		return d.pgdb.Stats().WaitDuration.String(), nil
	default:
		return d.lvldb.Stat(property)
	}
}
...
func (d *Database) Compact(start []byte, limit []byte) error {
	return d.lvldb.Compact(start, limit)
}
...
func (d *Database) NewIteratorWithStart(start []byte) ethdb.Iterator {
	return d.lvldb.NewIteratorWithStart(start)
}

// NewIteratorWithPrefix creates a binary-alphabetical iterator over a subset
// of database content with a particular key prefix.
func (d *Database) NewIteratorWithPrefix(prefix []byte) ethdb.Iterator {
	return d.lvldb.NewIteratorWithPrefix(prefix)
}

// Close satisfies the io.Closer interface
// Close closes the db connection
func (d *Database) Close() error {
	if err := d.lvldb.Close(); err != nil {
		return err
	}
	return d.pgdb.DB.Close()
}
